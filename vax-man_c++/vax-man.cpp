...
vector<array<float,3>> Pinky;
vector<array<float, 3>> Blinky;
vector<array<float, 3>> Inky;
vector<array<float, 3>> Clyde;
vector<vector<array<float,3>>> ghosts;
static int numbGhosts;
...
void init(void){
    ...
    Pinky.push_back(monster1);
    Blinky.push_back(monster2);
    Inky.push_back(monster3);
    Clyde.push_back(monster4);
    numbGhosts = Pinky.size() + Blinky.size() +Inky.size() + Clyde.size()
...
void drawMonster(&array<float,3> monster, float r, float g, float b){
	int x, y;
	glBegin(GL_LINES);
	glColor3f(r, g, b);
	//draw the head
...
void gameOver(){
	...
	if (numbGhosts >= 128 || Blinky.size() >= 32 || Pinky.size() >= 32 || Clyde.size() >= 32 || Inky.size() >= 32){
		over = true;
	}
	if (points == 106){
		over = true;
...
void multiplyGhosts(){
	...
	for (auto& x  : ghosts) {
        float monster;
		if (a == 0) { monster = monster1 } 
		else if (a == 1) { monster = monster2 }
		else if (a == 2) { monster= monster3 }
		else { monster = monster4 }
...
void display(){
	...
	if (replay){
		if (!over){
            currTime = GetTickCount() - startTime;
			drawLaberynth();
			...
			for (auto& x : ghosts) {
				...
				if (a == 0)		{ e= 0.0; f= 1.0; g= 1.0; } 
				else if (a == 1)	{ e= 1.0; f= 0.0; g= 0.0; }
				else if (a == 2)	{ e= 1.0; f= 0.0; g= 0.6; }
				else				{ e= 1.0; f= 0.3; g= 0.0; }

				for (auto& monster : list) {
					updateMonster(monster);
					drawMonster(monster, e, f, g);
				}
				a++;
			}
...
int main(int argc, char** argv){
	...
	glutCreateWindow("Vax-Man - by Kemo Sonko");
